...
bool eval_circuit(int id, const vector<bool>& s) {
  ...
  if (code == -1) {
    assert(1 <= left  and  left <= ub_n_nodes);
    assert(1 <= right and right <= ub_n_nodes);
...
int size(int id) {
  if (circ.at(id).code != -1) return 0;
    ...
    return 1 + size(circ.at(id).lp) + size(circ.at(id).rp);
  }
}
...
void check_circuit() {
  ...
  int ss = size();
  if (ss != n_gates) {
    cout << "Error: circuit has size " << ss << " "
         << "but expexted size " << n_gates << endl;
    exit(EXIT_FAILURE);
  }
...
void help(string exe) {
  ...
  cout << "Usage: " << exe << " [options] [< nlsp_d_n_b.out]" << endl;
...
int main(int argc, char** argv) {
  ...
  if (n_gates == -1) {
    cout << "Circuit does not exist, nothing to check" << endl;
    exit(EXIT_SUCCESS);
  }
